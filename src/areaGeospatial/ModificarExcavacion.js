...
import { puntoDentroDeArea, dibujarPoligono, dibujarPuntosArea, verificarInclusionAreas } from './helpers';
...
export default class ModificarExcavacion extends Component {
  ...
  obtenerExcavacion = async () => {
    ...
    const coordenadasPuntoGps = this.dibujarPuntoGPS(excavacionCompleta.puntoGps.coordinates);

    const poligonoExcavacion = this.dibujarPoligonoArea(coordenadasExcavacion, 'green');
  ...
  dibujarPuntoGPS = puntoGPS => {
    new L.popup({ elevation: 260.0 })
      .setLatLng([puntoGPS[0]+.002, puntoGPS[1]+.001])
      .setContent("Punto GPS Excavacion")
      .addTo(this.state.map);
    const nuevoMarcador = new L.Marker([puntoGPS[0], puntoGPS[1]], { draggable: 'true' });
  ...
  setearCoordenadas = coordenadas => this.setState({ coordenadasExcavacion: coordenadas });

  setearMarcadores = marcadores => this.setState({ marcadoresPoligonoExcavacion: marcadores });
  ...
  verificarInclusionAreas = () => {
    return verificarInclusionAreas(this.state.coordenadasExcavacion, this.state.coordenadasExploracion)
  };
  ...
  guardarAreaExcavacion = async () => {
    ...
    if (perteneceAExploracion) {
      const contienePuntoGps = puntoDentroDeArea(this.state.coordenadasPuntoGps.getLatLng(), this.state.coordenadasExcavacion);

      if (contienePuntoGps) {
        const body = {
          areaExcavacion: this.state.coordenadasExcavacion,
        };
        this.hacerRequest('modificarAreaExcavacion', 'poligonoExcavacion', body, 'Error al modificar el Area de Excavacion', 'coordenadasExcavacion', 'marcadoresPoligonoExcavacion');
      } else {
        toast.error('El area de Excavacion debe contenr al punto GPS de la excavacion');
        this.restaurarAreaOriginal();
      }
    } else {
      toast.error('El area de Excavacion debe estar contenida dentro del area de Exploracion');
      this.restaurarAreaOriginal();
    }
  };

  restaurarAreaOriginal = () => {
    const poligonoActual = this.dibujarPoligonoArea(this.state.coordenadasExcavacion);
    this.state.map.removeLayer(poligonoActual);

    this.state.marcadoresPoligonoExcavacion.forEach(marcador => this.state.map.removeLayer(marcador));
    const poligono = this.dibujarPoligonoArea(this.state.areaExcavacion, 'green');
  ...
  guardarPuntoGPSExcavacion = async () => {
    const pertenece = puntoDentroDeArea(this.state.coordenadasPuntoGps, this.state.coordenadasExcavacion);
    if (pertenece) {
      const body = {
        puntoGPSExcavacion: this.state.coordenadasPuntoGps,
      };
      this.hacerRequest(null, null, body, 'Error al modificar el punto GPS del Area de Excavacion');
    } else {
      toast.error('El punto GPS debe estar contenido dentro del area de Excavacion');
      this.setState({
  ...
  hacerRequest = async (area, poligono, body, msjError, coordenadasArea, marcadoresPoligono) => {
    const response = await fetch(`/api/areaExcavacion/${this.state.idExcavacion}`, {
      method: 'PUT',
      headers: {
        'Content-Type': 'application/json',
      },
    ...
    if (response.status !== 200) {
      ...
      if (coordenadasArea) {
        this.state[marcadoresPoligono].forEach(marcador => this.state.map.removeLayer(marcador));
        const poligonoArea = this.dibujarPoligonoArea(this.state[coordenadasArea], 'green');
        ...
        <div className="contenedorMenu">
          <div className="button-container">
            { !this.state.modificarAreaExcavacion ?
              <button className="btn btn-outline-primary" onClick={this.modificarAreaExcavacion}>
                Modificar Area Excavacion
              </button>
              :
              <button disabled={this.state.areaDisabled} className="btn btn-outline-primary" onClick={this.guardarAreaExcavacion}>
                Guardar Area Excavacion
              </button>
            }
          </div>
          <div className="button-container">
            <button className="btn btn-outline-primary" onClick={this.guardarPuntoGPSExcavacion}>
              Guardar Punto GPS Excavacion
            </button>
          </div>
          <Descripcion />
        </div>
