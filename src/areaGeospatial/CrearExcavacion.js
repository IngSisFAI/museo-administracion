...
export default class CrearExcavacion extends Component {
  ...
  onClickMap = event => {
    ...
    const marker = new L.Marker([event.latlng.lat, event.latlng.lng], { draggable: 'true' }).addTo(this.state.map);
    
    this.setState({
      points: [...this.state.points, { lat, lng }],
      markers: [...this.state.markers, marker],
      setAreaDisabled: !!this.state.areaExcavacion,
      setPuntoGpsDisabled: !this.state.areaExcavacion && !this.state.puntoGPSExcavacion,
    });
  };

  obtenerExploracion = async () => {
    const exploracionId = "5cfe8d0c385be24fc56da297";
    ...
    dibujarPoligono(exploracion.areaExploracion.locacion.coordinates[0], '', this.state.map);
    exploracion.excavaciones.forEach(excavacion => {
      dibujarPoligono(excavacion.areaExcavacion.locacion.coordinates[0], 'green', this.state.map);
    });
  }

  setearPuntoExcavacion = () => {
    if (this.state.areaExcavacion && !this.state.puntoGPSExcavacion && this.state.points[0]) {
      ...
      if (pertenece) {
        ...
        toast.error('Error: El punto GPS de la Excavacion debe estar contenido dentro del area')
        this.setState({ 
          points: [],
        });
  ...
  setearAreaExcavacion = () => {
    ...
    if (this.state.areaExploracion) {
      for (var i = 0; i < areaExcavacion.length && pertenece; i++) {
        pertenece = puntoDentroDeArea(areaExcavacion[i], this.state.areaExploracion);
      }
    }

    if (pertenece) {
      if (this.state.puntoGPSExcavacion) {
        pertenece = puntoDentroDeArea(this.state.puntoGPSExcavacion, areaExcavacion);

        if (!pertenece) {
          toast.error('Error: El area de la Excavacion debe contener al punto GPS de excavacion');
          this.setState({ 
            points: [],
          });
          this.state.markers.forEach(marker =>  this.state.map.removeLayer(marker));
      ...
      toast.error('Error: El area de la Excavacion debe estar contenida dentro del area de la Exploracion')
      this.setState({ 
        points: [],
      });
      this.state.markers.forEach(marker =>  this.state.map.removeLayer(marker));
  ...
  altaExcavacion = async () => {
    if (!this.state.areaExcavacion) {
      return toast.error('El area de Excavacion debe estar seteada para su creacion');
    }
    const body = {
    ...
    return (
      <div style={{ position: 'relative' }}>
        ...
        <div className="contenedorMenu">
          Hacer click en el mapa para setear los puntos limites del nuevo Area de Excavacion y su nuevo Punto GPS
          <div className="button-container">
            <button disabled={this.state.setAreaDisabled} className="btn btn-outline-primary" onClick={this.setearAreaExcavacion}>
              Setear nueva Area
            </button>
          </div>
          <div className="button-container">
            <button  disabled={this.state.setPuntoGpsDisabled} className="btn btn-outline-primary" onClick={this.setearPuntoExcavacion}>
              Setear nuevo Punto GPS
            </button>
          </div>
          <div className="button-container">
            <button disabled={!(this.state.puntoGPSExcavacion && this.state.areaExcavacion)} className="btn btn-outline-primary" onClick={this.altaExcavacion}>
              Alta Excavacion
            </button>
          </div>
