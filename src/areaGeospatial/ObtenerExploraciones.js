...
export default class ObtenerExcavacion extends Component {
  ...
  onClickMap = event => {
    ...
    if (datosAreaExcavacion) {
      const datosExcavacion = `<p><b>Area Excavacion</b>
      <br />Nombre Area: ${datosAreaExcavacion.nombre}
      <br />Ciudad: ${datosAreaExcavacion.ciudad}
      <br />Provincia: ${datosAreaExcavacion.provincia}
      <br />Pais: ${datosAreaExcavacion.pais}</p>`;
      ...
      if (datosAreaExploracion) {
        const datosExploracion = `<p><b>Area Exploracion</b>
        <br />Nombre Area: ${datosAreaExploracion.nombre}
        <br />Fecha: ${new Date(datosAreaExploracion.fecha)}
        <br />Ciudad: ${datosAreaExploracion.ciudad}
        <br />Provincia: ${datosAreaExploracion.provincia}
        <br />Pais: ${datosAreaExploracion.pais}</p>`;
  ...
  obtenerTodasExploraciones = async () => {
    ...
    if (exploracionesCompletas.length) {
      ...
      exploracionesCompletas.forEach(exploracionCompleta => {
        let coordenadasExploracion = exploracionCompleta.areaExploracion ? exploracionCompleta.areaExploracion.locacion.coordinates[0] : [];
        ...
        exploracionCompleta.excavaciones.forEach(excavacion => {
          ...
          const marcador = this.dibujarPuntosGPS(excavacion.puntoGps.coordinates);
          const datosPuntoGps = {
            ciudad: 'Neuquen',
            provincia: 'Neuquen',
            pais: 'Argentina',
        ...
        const datosExploracion = {
          nombre: exploracionCompleta.areaExploracion ? exploracionCompleta.areaExploracion.nombre : '',
          ciudad: exploracionCompleta.areaExploracion ? exploracionCompleta.areaExploracion.ciudad : 'Neuquen',
          provincia: exploracionCompleta.areaExploracion ? exploracionCompleta.areaExploracion.provincia : 'Neuquen',
          pais: exploracionCompleta.areaExploracion ? exploracionCompleta.areaExploracion.pais : 'Argentina',
